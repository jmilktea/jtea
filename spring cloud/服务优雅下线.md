...
public class ShutdownRegistry {
	...
	public final static List<EventBus> buses = Lists.newArrayList();
...
public class JobShutdown {
	...
	private static ConcurrentHashMap<String, Shutdown> map = new ConcurrentHashMap<>();

	@Around(value = "@annotation(xxlJob)")
	private <T> ReturnT<T> before(ProceedingJoinPoint joinPoint, XxlJob xxlJob) throws Throwable {
		map.computeIfAbsent(xxlJob.value(), s -> new Shutdown());
		if (map.get(xxlJob.value()).shutdown) {
			return new ReturnT<>(500, "application shutdown");
		}
		return (ReturnT<T>) joinPoint.proceed(joinPoint.getArgs());
	}
}
```
