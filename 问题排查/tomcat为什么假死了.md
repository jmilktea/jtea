...
对应到reactor模型，可以看成它是一种单reactor多线程模型，Acceptor线程负责建立连接，然后将建立好的连接注册到Poller，由Poller进行读写。Poller读写后创建请求，将其交给请求处理线程池。     
Acceptor和Poller线程对应的run方法都是一个死循环，源源不断的接收连接、读写连接。  

从reactor模型上看，在多核cpu下，多reactor多线程模型可以获得更高的效率，但tomcat10以下默认只能有一个Acceptor线程。  
